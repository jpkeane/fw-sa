...
RSpec.describe UserEmailAddress, type: :model do
  describe 'attributes' do
    ...
    it 'should ensure that the first email address added for a user is primary' do
      ...
      email1 = FactoryGirl.create(:user_email_address, user: user)
      expect(email1.primary?).to be_truthy
      email2 = FactoryGirl.create(:user_email_address, user: user)
  ...
  describe 'validations' do
    ...
    it 'should check valid email addresses' do
      ...
      invalid_addresses.each do |invalid_address|
        contact = FactoryGirl.build(:user_email_address, email_address: invalid_address)
    ...
    it 'should check that there can only be one primary email' do
      user = FactoryGirl.create(:user)
      FactoryGirl.create(:user_email_address, user: user, primary: true)

      second_email = FactoryGirl.build(:user_email_address, user: user, primary: true)
  ...
  describe 'methods' do
    it '#make_primary should make email primary and remove others' do
      user = FactoryGirl.create(:user)
      first_email = FactoryGirl.create(:user_email_address, user: user)
      second_email = FactoryGirl.create(:user_email_address, user: user)

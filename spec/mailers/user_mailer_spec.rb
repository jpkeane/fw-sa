...
RSpec.describe UserMailer, type: :mailer do
  ...
  describe '#password_reset_successful_email' do
    let!(:user) { FactoryGirl.create(:user) }
    let!(:email) { FactoryGirl.create(:email_address, user: user) }
    let!(:mail) { UserMailer.password_reset_successful_email(user) }
    ...
    it 'renders the headers' do
      expect(mail.subject).to eq('Floworx SysArch - Password has been reset')
      expect(mail.to).to eq([email.email_address])
      expect(mail.from).to eq(['do_not_reply@jpkeane.com'])
    end
